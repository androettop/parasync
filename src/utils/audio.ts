...
export class SongAudioManager implements Loadable<{}> {
  ...
  constructor(songTrackPaths: string[], drumsTrackPaths: string[]) {
  ...
  async load() {
    ...
    this._duration = st.duration_secs ?? -1;
  ...
  async refreshStatus() {
    const st = await window.__TAURI_INTERNALS__.invoke("audio_status");
    this._duration = st.duration_secs ?? this._duration;
    this._position = st.position_secs ?? this._position;
    this._isPlaying = !!st.is_playing;
  }
}
